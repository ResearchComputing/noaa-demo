...
# load data
smap <- raster('output/geotiffs/smap.tif')
...
s1 <- ggplot(eco_points, aes(x = smap, y = viirs_ndvi - min(viirs_ndvi))) +
  ...
  theme_minimal()
ggsave('output/plots/scatter1.pdf', plot = s1, width = 6, height = 4)
...
ggsave('output/plots/scatter2.pdf', plot = s2, width = 12, height = 8)
...
ggsave(file = "output/plots/levelplots.pdf", plot = g, width = 12, height = 8, units = 'in')

pdf(file = 'output/plots/surfs.pdf', width = 8, height = 6)
